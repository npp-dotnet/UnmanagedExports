...
// Type: RGiesecke.DllExport.MSBuild.ExportTaskImplementation`1
...
{
    public class ExportTaskImplementation<TTask> : IInputValues, IServiceContainer, IServiceProvider
      where TTask : IDllExportTask, ITask
    {
        ...
        private Func<Version, string, string> GetGetToolPath(string methodName)
        {
            ...
            {
                Func<Version, string, string> toolPathInternal;
                if (!ExportTaskImplementation<TTask>._GetFrameworkToolPathByMethodName.TryGetValue(methodName, out toolPathInternal))
                    ExportTaskImplementation<TTask>._GetFrameworkToolPathByMethodName.Add(methodName, toolPathInternal = this.GetGetToolPathInternal(methodName));
                return toolPathInternal;
            }
        }

        private static Func<string, int, string> WrapGetToolPathCall<TTargetDotNetFrameworkVersion>(
        ...
        private bool ValidateToolPath(
          string toolFileName,
          string currentValue,
          Func<Version, string, string> getToolPath,
        ...
        private bool TryToGetToolDirForFxVersion(
          string toolFileName,
          Func<Version, string, string> getToolPath,
          ref string toolDirectory)
        {
